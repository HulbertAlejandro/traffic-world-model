...
CHECKPOINT_PATH = ROOT_DIR / "models" / "checkpoints" / "raw_state" / "world_model_raw_best.pt"
RESULTS_DIR = ROOT_DIR / "results"
TEST_LATENT_PATH = PROCESSED_DIR / "test_raw_seq.npz"
...
def main() -> None:
    ...
    if not TEST_LATENT_PATH.exists():
        raise FileNotFoundError(
            f"Missing {TEST_LATENT_PATH}. Run scripts/prepare_raw_sequence_dataset.py first."
        )
    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(
            f"Missing {CHECKPOINT_PATH}. Run training/train_world_model_raw.py first."
        )
    ...
    report_path = RESULTS_DIR / "world_model_raw_evaluation.json"
    plot_path = RESULTS_DIR / "world_model_raw_compounding_error.png"
