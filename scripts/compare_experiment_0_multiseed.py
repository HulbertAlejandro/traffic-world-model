...
def evaluate(checkpoint: Path, test_path: Path, device: torch.device) -> dict[int, dict[str, float]]:
    ...
    return aggregate_by_horizon(records, hparams["latent_dim"])
...
def main(argv: list[str] | None = None) -> None:
    ...
    for z_dir, raw_dir in zip(args.z_dirs, args.raw_dirs):
        ...
        z = evaluate(z_dir / "world_model_best.pt", PROCESSED_DIR / "test_latent.npz", device)
        raw = evaluate(raw_dir / "world_model_raw_best.pt", PROCESSED_DIR / "test_raw_seq.npz", device)
