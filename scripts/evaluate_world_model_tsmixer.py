"""Evaluate the trained latent-dynamics World Model (LatentDynamicsTSMixer).

Experimento 3: mirror of scripts/evaluate_world_model.py, same test split,
same horizons and same persistence baseline, pointing at the TSMixer
...
CHECKPOINT_PATH = ROOT_DIR / "models" / "checkpoints" / "world_model_tsmixer_best.pt"
...
def main() -> None:
    ...
    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(
            f"Missing {CHECKPOINT_PATH}. Run training/train_world_model_tsmixer.py first."
        )
    ...
    if one_step["model_latent_mse"] < one_step["baseline_latent_mse"]:
        print("El World Model (TSMixer) SUPERA al baseline persistente a un paso (latente).")
    else:
        print("ADVERTENCIA: el World Model (TSMixer) NO supera al baseline persistente a un paso (latente).")

    if one_step["model_reward_mse"] < one_step["baseline_reward_mse"]:
        print("El World Model (TSMixer) SUPERA al baseline persistente a un paso (recompensa).")
    else:
        print("ADVERTENCIA: el World Model (TSMixer) NO supera al baseline persistente a un paso (recompensa).")
    ...
    report_path = RESULTS_DIR / "world_model_tsmixer_evaluation.json"
    plot_path = RESULTS_DIR / "world_model_tsmixer_compounding_error.png"
    save_metrics_report(summary, report_path)
    plot_compounding_error(summary, plot_path, model_label="World Model (TSMixer)")
