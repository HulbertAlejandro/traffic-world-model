...
**Resultado principal, con evidencia estadística**: con el presupuesto original del RL directo (13,000 interacciones reales por semilla), el controlador entrenado dentro del World Model superó al entrenado directamente contra SUMO en desempeño medio y fue mucho más consistente entre semillas (verificado con 3 semillas de entrenamiento independientes por método, evaluadas en SUMO real), usando aproximadamente un tercio de las interacciones reales con el simulador. La ventaja es clara a nivel de episodio individual (4.56 errores estándar de diferencia), aunque no alcanza significancia estadística formal a nivel de semilla de entrenamiento (p = 0.145 con solo 3 semillas por método). Una verificación posterior (Sección 20.2) precisa el alcance de este resultado: con el triple de presupuesto, el RL directo alcanza un desempeño comparable, pero consumiendo **~8.5 veces más interacciones reales** que el World Model. La ventaja demostrada del World Model es, por tanto, de eficiencia: alcanza el mismo nivel de control con una fracción de las interacciones con el simulador.
...
**Respuesta obtenida**: sí, con matices. Con el presupuesto original del RL directo, el controlador entrenado dentro del World Model no solo mantuvo el desempeño frente al entrenado directamente: lo superó, de forma consistente entre semillas, con aproximadamente un tercio de las interacciones reales. Esa ventaja, aunque clara a nivel de episodio, no alcanza el umbral de significancia estadística formal a nivel de semilla con el tamaño de muestra usado (3 semillas por método). Al triplicar el presupuesto del RL directo, este alcanza un desempeño comparable, pero con ~8.5 veces más interacciones reales que el World Model (Sección 20.2). La respuesta precisa es, entonces, que el World Model **reduce las interacciones necesarias sin deteriorar el desempeño**: logra el mismo nivel de control con una fracción del contacto con el simulador, tal como plantea la pregunta.
...
**Experimento 2 — Control por imaginación.** Se comparó el controlador entrenado sin World Model (RL directo) frente al que usa el modelo aprendido (entrenado en el sueño), ambos medidos en SUMO real. **Resultado: con el presupuesto original del RL directo, el controlador del World Model lo supera de forma consistente (Sección 2), con la salvedad de significancia estadística a nivel de semilla ya mencionada. Con el triple de presupuesto, el RL directo alcanza un desempeño comparable, pero con ~8.5 veces más interacciones reales (Sección 20.2).**
...
## 20.2 Verificación: RL directo con 3x presupuesto

La comparación del Experimento 2 dejó una limitación declarada: el RL directo se entrenó con un presupuesto deliberadamente modesto (10,000 pasos reales de entrenamiento), y no se podía descartar que más presupuesto cerrara la brecha. Para responderlo, las 3 semillas del RL directo se reentrenaron con **30,000 pasos**, cambiando solo el presupuesto (mismo script, misma normalización, misma selección del mejor checkpoint por evaluación periódica en SUMO real). Los modelos de 30,000 pasos no reemplazan a los oficiales: son una verificación. Se evaluaron con el mismo protocolo (30 episodios por semilla), y en la misma corrida se reprodujeron exactamente los resultados ya documentados del World Model y del RL directo de 10,000 pasos.

**Resultado del RL directo al triplicar el presupuesto:**

| | Media (90 episodios) | Mediana | Desviación de las medias por semilla | Episodios catastróficos (< -600) | Interacciones reales por semilla |
| --- | --- | --- | --- | --- | --- |
| RL directo, 10,000 pasos | -453.74 | -381.75 | 96.2 | 23/90 | 13,000 |
| RL directo, 30,000 pasos | -335.24 | -271.60 | 30.6 | 11/90 | 39,000 |
| World Model (PPO del sueño) | -326.79 | -293.00 | 19.3 | 5/90 | ~4,600 |

Con 30,000 pasos, la semilla que antes colapsaba a la regla trivial ("pedir siempre la fase contraria") sale de esa meseta y deja de colapsar. Con 10,000 pasos, su mejor checkpoint se había quedado en ese óptimo local temprano. Frente al World Model, la diferencia de medias baja de 126.95 a 8.46 puntos y deja de ser significativa (Welch p = 0.74 a nivel de episodio y p = 0.71 a nivel de semilla). El RL directo gana además la mayoría de las comparaciones episodio a episodio sobre el mismo escenario (159 de 270) y tiene mejor mediana. El World Model conserva menos episodios catastróficos (5 frente a 11 de 90), una diferencia que no se sometió a prueba.

**Contabilidad de interacciones.** Cada semilla del RL directo con 30,000 pasos consume 39,000 interacciones reales: 30,000 de entrenamiento más 9,000 de la evaluación periódica que selecciona su mejor checkpoint. El World Model consume ~4,600 por semilla: el dataset de 4,800 transiciones, recolectado una sola vez y compartido por las 3 semillas, más 3,000 de selección. **El RL directo necesita ~8.5 veces más interacciones reales para alcanzar el mismo nivel de control.**

**Salvedad metodológica.** Con más presupuesto, el RL directo también elige su mejor checkpoint entre más evaluaciones periódicas (30 en vez de 10), sobre los mismos 5 escenarios de selección. La mejora se confirma en los 30 escenarios de prueba, que son independientes de los de selección, así que no es un artefacto. Pero parte de la ganancia puede venir de esa búsqueda más amplia y no solo de más entrenamiento, y esta verificación no separa ambos efectos.

**Interpretación.** La verificación **precisa** el resultado del Experimento 2 en vez de corregirlo. Con el presupuesto original del RL directo (13,000 interacciones por semilla, ~2.8 veces las del World Model), el World Model tiene mejor desempeño medio y es mucho más consistente entre semillas. Con el triple de presupuesto, el RL directo alcanza un desempeño comparable, pero consumiendo ~8.5 veces más interacciones reales. La ventaja demostrada del World Model es, por tanto, de **eficiencia en interacciones**: llega al mismo nivel de control con una fracción del contacto con el simulador, que es exactamente lo que plantea la pregunta de investigación (Sección 4). No es una ventaja de control cuando el RL directo dispone de presupuesto de sobra. Queda sin medir la curva completa de desempeño frente a interacciones del RL directo: solo hay dos puntos, 13,000 y 39,000 por semilla.
...
- **H3.** El uso de un World Model puede reducir la cantidad de interacciones necesarias con SUMO para obtener un controlador competitivo respecto al entrenamiento directo. **Confirmada**, en el sentido preciso de la hipótesis: el World Model obtiene un controlador competitivo con muchas menos interacciones reales. Con el presupuesto original del RL directo (~2.8 veces las interacciones del World Model), el World Model tiene mejor desempeño, con una ventaja clara a nivel de episodio pero no significativa a nivel de semilla (3 por método). Con el triple de presupuesto, el RL directo alcanza un desempeño comparable, pero necesita ~8.5 veces las interacciones del World Model (Sección 20.2). La confirmación es de eficiencia en interacciones, no de mejor control cuando el RL directo dispone de presupuesto de sobra.
...
- Una conclusión experimental sobre el valor de usar un World Model para reducir interacciones con el simulador: **positiva**. Con el presupuesto original del RL directo, el World Model controla mejor con aproximadamente un tercio de las interacciones; con el triple de presupuesto, el RL directo lo alcanza, pero consumiendo ~8.5 veces más interacciones reales (Sección 20.2). Los matices de significancia estadística se declaran explícitamente, no se ocultan.
...
- Con solo 3 semillas de entrenamiento por método, la ventaja del World Model sobre el RL directo con el presupuesto original, aunque consistente, no alcanza significancia estadística formal a nivel de semilla. Con el triple de presupuesto, esa ventaja de desempeño desaparece (Sección 20.2); la ventaja que se mantiene es la de interacciones reales.
