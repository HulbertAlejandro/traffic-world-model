"""Training loop for the TSMixer latent dynamics model (Experimento 3).
...
def main() -> None:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        save_checkpoint(model, config, CHECKPOINT_DIR / "world_model_tsmixer_last.pt")
        if validation_loss < best_validation_loss:
            best_validation_loss = validation_loss
            save_checkpoint(model, config, CHECKPOINT_DIR / "world_model_tsmixer_best.pt")
            epochs_without_improvement = 0
        else:
    ...
    plt.title("World model (TSMixer) training loss")
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "world_model_tsmixer_loss.png")
    plt.close()
