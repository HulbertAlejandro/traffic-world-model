...
def test_transformer_predicts_next_latent_and_reward() -> None:
    config = _make_config()
...
def test_transformer_rejects_invalid_latent_dim() -> None:
    with pytest.raises(ValueError, match="latent_dim must be positive"):
        LatentDynamicsTransformer(latent_dim=0, action_dim=2)


def test_transformer_rejects_invalid_action_dim() -> None:
    with pytest.raises(ValueError, match="action_dim must be positive"):
        LatentDynamicsTransformer(latent_dim=8, action_dim=0)


def test_transformer_rejects_d_model_not_divisible_by_nhead() -> None:
    with pytest.raises(ValueError, match="divisible by nhead"):
        LatentDynamicsTransformer(latent_dim=8, action_dim=2, d_model=30, nhead=4)


def test_transformer_rejects_sequence_length_different_from_configured() -> None:
    model = LatentDynamicsTransformer(latent_dim=8, action_dim=2, sequence_length=5)
...
def test_transformer_rejects_mismatched_sequence_length() -> None:
    model = LatentDynamicsTransformer(latent_dim=8, action_dim=2)
    latent_sequence = torch.randn(2, 4, 8)
...
def test_transformer_rejects_mismatched_batch_size() -> None:
    model = LatentDynamicsTransformer(latent_dim=8, action_dim=2)
    latent_sequence = torch.randn(2, 4, 8)
